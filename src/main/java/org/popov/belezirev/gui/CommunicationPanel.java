...
public class CommunicationPanel extends JPanel {
    ...
    private Credentials<String, String> getClientCredentials() {
        ...
        panel.add(Box.createRigidArea(new Dimension(SIGN_IN_PANEL_COMPONETS_SEPARATOR_WIDTH, SIGN_IN_PANEL_COMPONETS_SEPARATOR_HEIGHT)));
        panel.add(passwordAreaLabel);
        panel.add(passwordArea);
        final int selectedOption = JOptionPane.showOptionDialog(null, panel, "SIGN-IN", JOptionPane.NO_OPTION, JOptionPane.PLAIN_MESSAGE,
            null, options, options[0]);
        final String username = usernameArea.getText();
        final char[] password = passwordArea.getPassword();
        if (selectedOption == OK_BUTTON && isUserNameValid(username) && isCredentialValidPassword(password)) {
            return new Credentials<>(username, password.toString());
        }
    ...
    private boolean isCredentialValidPassword(char[] password) {
    ...
    private Boolean isUserNameValid(final String credential) {
    ...
    private JList<String> createOnlineUsersDisplayList() {
        ...
        onlineUsersDisplayList.setBorder(new EmptyBorder(TOP_MARGIN_OFFSET, LEFT_MARGIN_OFFSET, BOTTOM_MARGIN_OFFSET, RIGHT_MARGIN_OFFSET));
